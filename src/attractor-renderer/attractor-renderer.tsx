...
export class AttractorRenderer extends React.Component<AttractorRendererProps> {
  render() {
    ...
    return (
      <g className={classNames('attractor-renderer', { positive: mass > 0, negative: mass < 0 })}>
        <circle className="background" cx={x} cy={y} r={Math.min(20, Math.abs(mass))} />

        <g className="event-target-container" onClick={onClick}>
          <circle className="event-target" cx={x} cy={y} r="20" />
        </g>
